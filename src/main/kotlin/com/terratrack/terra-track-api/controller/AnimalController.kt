...
import com.terratrack.`terra-track-api`.config.toUser
import com.terratrack.`terra-track-api`.dto.AnimalDto
import com.terratrack.`terra-track-api`.entity.Animal
import com.terratrack.`terra-track-api`.service.AnimalService
...
class AnimalController(private val animalService: AnimalService) {
    ...
    fun getAnimals(authentication: Authentication): ResponseEntity<List<AnimalDto>> {
        ...
        return ResponseEntity(
            animalService.findAnimalsByOwnerId(authUser.userId)
                .map { animal: Animal -> AnimalDto(animal.animalId, animal.name, animal.species) },
    ...
    fun getAnimal(authentication: Authentication, @PathVariable animalId: String): ResponseEntity<AnimalDto?> {
        ...
        return ResponseEntity(AnimalDto(animal.animalId, animal.name, animal.species), HttpStatusCode.valueOf(200))
    }
}
